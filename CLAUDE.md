...
- `js/look.js` — loaded last: art layer over world.js (plants, grass, moss, decor, rock + cork-wall textures `CORKWALL`) and `contactShadows()` (instanced soft shadows under body, leg tips, prey; called from `loop`). Grade pass also has lens fringe `uCA`.
